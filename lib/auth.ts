...
export class AuthService {
  ...
  static async signup(data: {
    ...
    try {
      // Rate limiting
      const rateLimitResult = await rateLimit.check(`signup:${data.email}`, 5, 3600000) // 5 per hour
      if (!rateLimitResult.allowed) {
        throw new AuthError('Too many signup attempts. Please try again later.', 429)
      }
      ...
      const passwordValidation = PasswordManager.validateStrength(data.password)
      ...
      if (data.dob) {
        ...
        if (ageYears < 18) {
          throw new AuthError('You must be at least 18 years old to register', 400)
        }
      }
  ...
  static async login(data: {
    ...
    try {
      // Rate limiting
      const rateLimitResult = await rateLimit.check(`login:${data.email}`, 10, 900000) // 10 per 15 min
      if (!rateLimitResult.allowed) {
      ...
      if (user.lockedUntil && user.lockedUntil > new Date()) {
        throw new AuthError('Account temporarily locked. Please try again later.', 423)
      }
      ...
      // Verify password
      const isPasswordValid = await PasswordManager.verify(data.password, user.passwordHash)
      if (!isPasswordValid) {
        ...
        const updatedUser = await prisma.user.update({
          ...
          data: {
            ...
            lockedUntil:
              user.loginAttempts >= 4 ? new Date(Date.now() + 15 * 60 * 1000) : undefined, // 15 min lock
          },
        })
