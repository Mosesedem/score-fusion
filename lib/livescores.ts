...
export class LiveScoreService {
  ...
  async createDefaultSports(): Promise<void> {
    const defaultSports = [
      ...
      { name: 'basketball', displayName: 'Basketball', sortOrder: 2, icon: '🏀' },
      ...
      { name: 'volleyball', displayName: 'Volleyball', sortOrder: 10, icon: '🏐' }
    ]
  ...
  private async updateMatchesFromProvider(provider: LiveScoreProvider): Promise<void> {
    switch (provider.name) {
      ...
      case 'the_sports_db':
        ...
        break
      default:
        console.log(`No update method implemented for provider: ${provider.name}`)
    }
  }

  private async updateFromApiFootball(provider: LiveScoreProvider): Promise<void> {
    if (!provider.config?.sports || !Array.isArray(provider.config.sports)) {
      return
    }
    ...
    for (const sportCode of provider.config.sports) {
      try {
        // Get live matches for the sport
        const response = await fetch(`${apiUrl}/fixtures?live=all&sport=${sportCode}`, {
        ...
        for (const matchData of matches) {
          await this.processApiFootballMatch(matchData, provider)
        }
  ...
  private async processApiFootballMatch(matchData: any, provider: LiveScoreProvider): Promise<void> {
    try {
      const sport = await this.getSportByCode(matchData.sport?.name)
      if (!sport) return

      const league = await this.getOrCreateLeague(matchData.league?.name, sport.id)
      if (!league) return
      ...
      const matchUpdateData = {
        ...
        scheduledAt: new Date(matchData.fixture?.date),
  ...
  private async updateFromTheSportsDB(provider: LiveScoreProvider): Promise<void> {
    ...
    try {
      ...
      const sports = ['Football', 'Basketball', 'Tennis', 'Baseball', 'Hockey', 'MMA', 'Boxing']

      for (const sportName of sports) {
        const response = await fetch(`${apiUrl}/searchseason.php?s=${sportName}`, {
        ...
        for (const country of seasons.slice(0, 3)) { // Limit to avoid rate limits
          ...
          if (countrySeasons.length > 0) {
            ...
            if (eventsResponse.ok) {
              ...
              for (const eventData of events.slice(0, 10)) { // Limit events per sport
                await this.processTheSportsDBEvent(eventData, sportName, provider)
              }
            }
          }
        }
  ...
  private async processTheSportsDBEvent(eventData: any, sportName: string, provider: LiveScoreProvider): Promise<void> {
    try {
      ...
      const matchUpdateData = {
        ...
        scheduledAt: eventData.dateEvent ? new Date(eventData.dateEvent) : new Date(),
  ...
  private async processMatchEvents(matchId: string, matchData: any): Promise<void> {
    try {
      ...
      for (const eventData of events) {
        if (eventData.type === 'Goal') {
          ...
          await prisma.matchEvent.create({
            data: {
              matchId,
              type: eventData.detail === 'Yellow Card' ? 'yellow_card' : 'red_card',
              team: eventData.team === 'home' ? 'home' : 'away',
              minute: eventData.time?.elapsed,
              player: eventData.player?.name,
  ...
  private async getSportByCode(code?: string): Promise<any> {
    if (!code) return null
  ...
  private async getSportByName(name: string): Promise<any> {
    return prisma.sport.findFirst({
      where: {
        OR: [
  ...
  private async getOrCreateLeague(leagueName: string, sportId: string): Promise<any> {
  ...
  private mapApiFootballStatus(status?: string): string {
    switch (status) {
      ...
      case 'POSTPONED': case 'SUSP': return 'postponed'
      case 'CANCELLED': case 'ABD': return 'cancelled'
  ...
  private getCurrentMinute(elapsed?: number): number | null {
    return elapsed || null
  }
  ...
  async getMatchById(matchId: string): Promise<LiveScoreMatch | null> {
