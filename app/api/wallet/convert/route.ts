...
export async function POST(request: NextRequest) {
  try {
    ...
    const rateLimitResult = await rateLimit.check(`convert:ip:${ip}`, 5, 300000) // 5 per 5 min

    if (!rateLimitResult.allowed) {
      return NextResponse.json(
        { success: false, error: 'Too many conversion attempts. Please try again later.' },
        { status: 429 }
      )
    }
    ...
    if (validatedData.tokens < MIN_CONVERSION_TOKENS) {
      return NextResponse.json(
        { success: false, error: `Minimum ${MIN_CONVERSION_TOKENS} tokens required for conversion` },
        { status: 400 }
      )
    }
    ...
    if (validatedData.tokens > MAX_CONVERSION_TOKENS) {
      return NextResponse.json(
        { success: false, error: `Maximum ${MAX_CONVERSION_TOKENS} tokens allowed per conversion` },
        { status: 400 }
      )
    }
    ...
    return NextResponse.json({
      ...
      data: {
        ...
        conversion: {
          ...
          estimatedValue: (wallet.tokens + wallet.bonusTokens) * TOKEN_CONVERSION_RATE,
        },
        statistics: {
