...
export async function POST(request: NextRequest) {
  try {
    ...
    const rateLimitResult = await rateLimit.check(`earnings:ip:${ip}`, 20, 300000) // 20 per 5 min
    ...
    switch (action) {
      case 'claim_daily_login':
        return await claimDailyLogin(userId, auth.user, ip, request.headers.get('user-agent'))

      case 'claim_achievement':
        return await claimAchievement(userId, data?.achievementId, ip, request.headers.get('user-agent'))

      case 'view_tip':
        return await awardTipView(userId, data?.tipId, ip, request.headers.get('user-agent'))

      case 'place_bet':
        return await awardBetPlacement(userId, data?.betId, data?.amount, ip, request.headers.get('user-agent'))
...
async function awardTipView(
  ...
  if (existingReward) {
    return NextResponse.json({ success: true, data: { alreadyRewarded: true } })
  }
