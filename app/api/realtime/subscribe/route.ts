...
export async function GET(request: NextRequest) {
  try {
    ...
    const rateLimitResult = await rateLimit.check(`realtime:ip:${ip}`, 20, 60000) // 20 per minute
...
function getAvailableChannels(user?: any): string[] {
...
export function subscribeToChannels(connectionId: string, channels: string[]): boolean {
  const connection = connections.get(connectionId)
...
export function unsubscribeFromChannels(connectionId: string, channels: string[]): boolean {
  const connection = connections.get(connectionId)
...
export async function broadcastToChannel(channel: string, message: any): Promise<number> {
