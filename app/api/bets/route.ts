...
const placeBetSchema = z.object({
  ...
  amount: z.number().positive('Amount must be positive').max(10000, 'Maximum amount is 10,000'),
  ...
  try {
    ...
    if (!rateLimitResult.allowed) {
      return NextResponse.json(
        { success: false, error: 'Too many bet attempts. Please try again later.' },
        { status: 429 }
      )
    }
    ...
    if (userProfile?.selfExclusionUntil && userProfile.selfExclusionUntil > new Date()) {
      return NextResponse.json(
    ...
    if (userProfile?.depositLimits) {
      ...
      const limits = userProfile.depositLimits as any
      ...
      if (limits.weekly && weeklyAmount + validatedData.amount > limits.weekly) {
        return NextResponse.json(
      ...
      if (limits.monthly && monthlyAmount + validatedData.amount > limits.monthly) {
        return NextResponse.json(
...
export async function GET(request: NextRequest) {
  try {
    ...
    const totalStaked = Number(
      ...
      stats.find(s => s.status === 'won')?._sum.amount || 0
    ) + Number(
      stats.find(s => s.status === 'lost')?._sum.amount || 0
    )

    const totalWon = Number(stats.find(s => s.status === 'won')?._sum.payout || 0)
    const totalLost = Number(stats.find(s => s.status === 'lost')?._sum.amount || 0)
    ...
    return NextResponse.json({
      ...
      data: {
        bets: bets.map(bet => ({
          ...
          potentialPayout: bet.odds ? Number(bet.amount) * Number(bet.odds) : null,
        })),
        pagination: {
